...
|---|---|---|---|
| 1 | **Reconstruct events / attribute outcomes** | Append-only ledger; every event is one immutable entry carrying `source_id`, recorder-assigned `seq` + `timestamp`, and the full payload. *(Example domain application: the bundled ROE schema adds structured fields for defense decision events.)* | [ledger.py:116](../src/ledger.py#L116) · [roe_schema.py](../src/roe_schema.py) |
